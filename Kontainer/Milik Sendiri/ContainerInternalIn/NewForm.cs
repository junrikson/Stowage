        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (IsValid())
            {
                try
                {
                    ...
                    {
                        query = "INSERT INTO containerInternalIn" +
                            "(code, date, containerCode, note, updated, created, username) VALUES " +
                            "(@code + (SELECT RIGHT('000' + CAST((ISNULL((SELECT TOP 1 RIGHT(code,4) " +
                            "   FROM containerInternalIn " +
                            "   WHERE LEFT(RIGHT(code,10),6) = @today " +
                            "   ORDER BY RIGHT(code,4) DESC),0) + 1) AS VARCHAR(4)),4))," +
                            "@date, @containerCode, @note, getdate(), getdate(), @username)";
                    }
                    else
                    {
                        query = "INSERT INTO containerInternalIn" +
                               "(code, date, containerCode, note, updated, created, username) VALUES " +
                               "(@code, @date, @containerCode, @note, getdate(), getdate(), @username)";
                    }
                    ...
                    using (SqlConnection con = new SqlConnection(connString))
                    {
                        ...
                        if (String.IsNullOrEmpty(txtCode.Text))
                        {
                            cmd.Parameters.Add("@code", SqlDbType.VarChar).Value = "IIN/" + dtpDate.Value.ToString("yyMMdd");
                        ...
                        cmd = new SqlCommand("UPDATE masterContainer SET status = 1, available = 1 WHERE code = @containerCode", con, tran);
                        cmd.Parameters.Add("@containerCode", SqlDbType.VarChar).Value = txtContainerCode.Text;
                        cmd.ExecuteNonQuery();
