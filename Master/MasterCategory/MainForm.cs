    ...
    public partial class MainForm : Syncfusion.Windows.Forms.MetroForm
    {
        ...
        private bool CheckAuth()
        {
            ...
            string query = "SELECT c.[active], c.[new], c.[edit], c.[view], c.[delete], c.[print], c.[refresh] " +
                ...
                "WHERE a.code = @username AND c.menuCode = 'A0004' ";
            try
            {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (gridView.SelectedCells.Count != 0)
            {
                try
                {
                    ...
                    DataGridViewRow row = cell.OwningRow;
                    id = row.Cells[0].Value.ToString();
                    name = row.Cells["Nama Kategori"].Value.ToString();
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        using (SqlConnection con = new SqlConnection(connString))
                        {
                            using (SqlCommand cmd = new SqlCommand("DELETE FROM masterCategory WHERE id = @id", con))
                            {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (cmbFilterColumn.SelectedItem.ToString() == "Nama Kategori")
            {
                name = txtFilterValue.Text;
            }
            else if (cmbFilterColumn.SelectedItem.ToString() == "Kode Kategori")
            {
        ...
        public void Grid_Load(int status = 1, string code = "", string name = "", string note = "")
        {
            DataTable dt = new DataTable();
            dt.Columns.Add("ID", typeof(int));
            dt.Columns.Add("Kode Kategori", typeof(String));
            dt.Columns.Add("Nama Kategori", typeof(String));
            dt.Columns.Add("Batas Bawah (TON)", typeof(Decimal));
            dt.Columns.Add("Batas Atas (TON)", typeof(Decimal));
            ...
            try
            {
                String sqlString = "SELECT id, code, name, min, max, note, status, username, updated, created FROM masterCategory WHERE " +
                    "status = (@status) " +
                ...
                for (int i = 0; i < tDT.Rows.Count; i++)
                {
                    ...
                    dr["ID"] = tDR["id"];
                    dr["Kode Kategori"] = tDR["code"];
                    dr["Nama Kategori"] = tDR["name"];
                    dr["Batas Bawah (TON)"] = tDR["min"];
                    dr["Batas Atas (TON)"] = tDR["max"];
        ...
        {
            cmbFilterColumn.Items.Add("Nama Kategori");
            cmbFilterColumn.Items.Add("Kode Kategori");
            cmbFilterColumn.Items.Add("Keterangan");
            cmbFilterColumn.SelectedItem = "Nama Kategori";
