...
namespace MasterShip
{
    public partial class MainForm : Syncfusion.Windows.Forms.MetroForm
    {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (gridView.SelectedCells.Count != 0)
            {
                try
                {
                    ...
                    DataGridViewRow row = cell.OwningRow;
                    id = row.Cells[0].Value.ToString();
                    name = row.Cells["Nama Kapal"].Value.ToString();
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        using (SqlConnection con = new SqlConnection(connString))
                        {
                            using (SqlCommand cmd = new SqlCommand("DELETE FROM masterShip WHERE id = @id", con))
                            {
        ...
        public void Grid_Load(int status = 1, string code = "", string name = "", string note = "")
        {
            ...
            dt.Columns.Add("Kode Kapal", typeof(String));
            dt.Columns.Add("Nama Kapal", typeof(String));
            ...
            try
            {
                String sqlString = "SELECT id, code, name, note, status, username, updated, created FROM masterShip WHERE " +
                    "status = (@status) " +
                ...
                for (int i = 0; i < tDT.Rows.Count; i++)
                {
                    ...
                    dr["ID"] = tDR["id"];
                    dr["Kode Kapal"] = tDR["code"];
                    dr["Nama Kapal"] = tDR["name"];
        ...
        {
            cmbFilterColumn.Items.Add("Nama Kapal");
            cmbFilterColumn.Items.Add("Kode Kapal");
            cmbFilterColumn.Items.Add("Keterangan");
            cmbFilterColumn.SelectedItem = "Nama Kapal";
