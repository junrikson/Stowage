    ...
    public partial class MainForm : Syncfusion.Windows.Forms.MetroForm
    {
        ...
        private bool CheckAuth()
        {
            ...
            string query = "SELECT c.[active], c.[new], c.[edit], c.[view], c.[delete], c.[print], c.[refresh] " +
                ...
                "WHERE a.code = @username AND c.menuCode = 'D0001' ";
            try
            {
        ...
        {
            NewForm newForm = new NewForm(this.username);
        ...
        private void btnView_Click(object sender, EventArgs e)
        {
            ...
            if (gridView.SelectedCells.Count != 0)
            {
                try
                {
                    ...
                    ViewForm viewForm = new ViewForm(id);
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            if (gridView.SelectedCells.Count != 0)
            {
                try
                {
                    ...
                    EditForm editForm = new EditForm(id, this.username);
            ...
            if (gridView.SelectedCells.Count != 0)
            {
                try
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        using (SqlConnection con = new SqlConnection(connString))
                        {
                            using (SqlCommand cmd = new SqlCommand("DELETE FROM masterMenu WHERE id = @id", con))
                            {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            else if (cmbFilterColumn.SelectedItem.ToString() == "Code")
            {
        ...
        public void Grid_Load(int status = 1, string code = "", string name = "")
        {
            ...
            try
            {
                String sqlString = "SELECT id, code, name, status, username, updated, created FROM masterMenu WHERE " +
                    "status = (@status) " +
        ...
        private void GridSettings()
        {
            ...
            cmbFilterColumn.Items.Add("Code");
            cmbFilterColumn.SelectedItem = "Name";
